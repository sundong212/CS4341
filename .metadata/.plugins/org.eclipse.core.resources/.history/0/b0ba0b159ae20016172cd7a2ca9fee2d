...
public class Heuristic5 {
	
	public int heu5(Terrain current, Terrain goal){
		...
		return Math.abs(goal.col_num-current.col_num)+Math.abs(goal.row_num-current.row_num)+1/3*findDirection(current, goal);
	}
